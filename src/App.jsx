...
function App() {
  ...
  const fetchAlbumData = async (id) => {
    ...
    try {
      const response = await axios.get(
        `https://studio.codnix.com/creation/ealbum/${id}.json`
      );
      ...
      setError(
        `Failed to load album ${id}. Please check the album code and try again.`
      );
      setLoading(false);
    }
  };
  ...
  return (
    <div
      className="relative min-h-screen w-full h-full overflow-hidden bg-zinc-800 text-white "
              ...
              <div className="fixed inset-0 flex items-center justify-center bg-black/90 z-40 p-4">
                <div className="bg-red-900/50 border border-red-800 rounded-lg p-4 sm:p-6 max-w-xs sm:max-w-sm md:max-w-md text-center">
                  ...
                  <p className="mb-4 text-sm sm:text-base">
                    There was an error loading the 3D viewer. Please try
                    refreshing the page.
                  </p>
                </div>
              </div>
